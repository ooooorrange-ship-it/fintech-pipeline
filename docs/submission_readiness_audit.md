...
| 检查项 | 状态 | 详情 |
|---|---|---|
| 完整源代码 | pass | 脚本数=29，缺失=[] |
| 文件夹readme.txt覆盖 | pass | 目录数=16，缺失=[] |
| Conda 环境文件 | pass | environment.yml |
| 部署文档 | pass | DEPLOYMENT.md |
| 项目说明 | pass | readme.md |
| 数据边覆盖审计 | pass | 56/59 缺边为原始数据事实，未伪造资金链路 |
| 可复现的清洗数据输入 | pass | 缺失=[] |
...
| 模型文件 overfit_guardrailed_final | pass | models/model13_guardrailed_final_strategy_A.json |
| 模型文件 rule_aware_calibration | pass | models/model14_rule_aware_guardrailed_strategy_A.json |
...
| 过拟合护栏最终权重可加载 | pass | weights={'图-树-时序融合模型': 1.0, 'Model12': 0.0} |
| 规则感知校准权重可加载 | pass | weights={'图-树-时序融合模型': 1.0, 'rule_score': 0.0} |
