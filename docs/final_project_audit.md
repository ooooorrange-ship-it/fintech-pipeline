...
| valid全量指标存在 | pass | 最终验证集选择模型报告文件存在 |
| test全量账户预测覆盖 | pass | 预测行=11087，账户数=11087，重复ID=0 |
| test全量指标存在 | pass | 最终验证集选择模型报告文件存在 |
...
| 关联账户节点ID可追溯 | pass | 记录数=72，未知账户=[] |
| 可疑路径节点ID可追溯 | pass | 记录数=154，未知账户=[] |
| 资金结构节点ID可追溯 | pass | 记录数=4，未知账户=[] |
| 数据边覆盖审计 | pass | 56/59 缺边已核验为原始数据事实，未伪造资金链路 |
| 任务2使用全量账户评估 | pass | evaluation_scope=all_accounts |
| 任务2 AUC达标 | pass | AUC=0.9857850279412036 |
| 任务2 Top5%召回达标 | pass | Top5%召回=0.9491525423728814 |
| 任务2最强传统基线PR-AUC提升20% | pass | 相对传统RF提升=0.5670974934847113 |
| 任务3关联命中或人工抽检指标 | pass | 关联命中率=不适用（56/59 缺边），已抽检=5/5，人工通过率=100.00% |
| 任务4典型案例不少于5个 | pass | 案例数=5 |
| 交付物关联行数一致 | pass | layered=72，deliverable=72，audit=72 |
| 交付物路径行数一致 | pass | layered=154，deliverable=154，audit=154 |
| 交付物资金结构行数一致 | pass | layered=4，deliverable=4，audit=4 |
