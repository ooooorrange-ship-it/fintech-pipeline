...
节点颜色：**红色=确认嫌疑人，橙色=受害人，蓝色=其它**，颜色即代表账户标签，节点下方仅显示账户 ID 与模型风险分；橙色箭头=转入，蓝色箭头=转出；**每条线=一笔真实交易**，线宽=单笔金额（对数）。边描述嵌在边线内：文字沿弧线方向排版，无底框，弧线在文字处断开、以白色描边保证可读性，显示单笔时间与金额；同一对手的重复交易分居弧线两侧并用不同弧度错开，避免文字遮挡节点。所有图谱只使用截至测试期截止（2025-12-31）的历史交易，未使用未来交易。
