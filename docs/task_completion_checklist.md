...
| 任务2 | 测试集 AUC >= 0.85 | 已完成 | 0.9858 |
| 任务2 | PR-AUC 较强XGB基线提升 >=20% | 已完成 | 177.03% |
| 任务2 | PR-AUC 较规则基线提升 >=20% | 已完成 | 22644.57% |
| 任务2 | Top5% 覆盖确认风险账户 >=50% | 已完成 | 94.92% |
...
| 任务3 | 人工抽检通过率 | 已完成 | 已抽检 5 个，通过率 100.00% |
...
注意：原始标签表没有标签时间，因此不能严格验证“未来新增标签”口径；当前已保证动态特征按 split cutoff 的滚动历史窗口构建，且标签字段不进入模型特征。