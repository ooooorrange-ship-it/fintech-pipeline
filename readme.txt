...
技术路线：滚动动态资金图谱特征 + 验证集选择融合（图-树-时序融合模型）+ 分层证据研判
...
| 任务2 | 图-树-时序融合模型融合模型与基线对比 | Test AUC 0.9858、PR-AUC 0.8049、Top5% 覆盖 56/59 |
| 任务3 | Top20 关联、多跳路径、资金结构、典型案例 | 人工抽检 5/5 通过 |
| 任务4 | 研判报告模板、样例、证据字典、一致性审计 | 报告至少引用两类可追溯证据 |

图-树-时序融合模型由验证集选择得到：0.75 x 动态图树融合模型 + 0.20 x CatBoost + 0.05 x 轻量 TGN。相对最强传统 RandomForest 基线，PR-AUC 提升约 56.71%。
