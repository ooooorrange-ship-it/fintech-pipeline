...
注意：当前图-树-时序融合模型（Graph-Tree-Temporal Champion Fusion）由验证集选择 `动态图树融合模型=0.75、CatBoost=0.20、轻量TGN=0.05`。动态图树融合模型内部为动态图 RandomForest 0.7、v6 动态融合 0.3、GraphSAGE 0。TGN 和 GraphSAGE 都保留为可复现的动态图模型对照分支。Model12 是五折正则化 Bagging 对照模型，用于检查过拟合和账户级泛化风险，不替代当前图-树-时序融合模型。Model13 是过拟合护栏配置，验证集结果选择 `图-树-时序融合模型=1.0、Model12=0.0`，因此主模型保持图-树-时序融合模型。Model14 是规则感知校准层，验证集结果选择 `图-树-时序融合模型=1.0、rule_score=0.0`，因此规则层只作为解释锚点，不改变最终风险分。
...
| `models/model8_final_dynamic_fusion_v7_strategy_A.json` | 动态图树融合模型（Dynamic Graph Tree Ensemble）配置 |
...
| `models/model11_validation_selected_best_strategy_A.json` | 图-树-时序融合模型（Graph-Tree-Temporal Champion Fusion）验证集选择配置 |
