...
模型分数来源拆解：56 个缺边账户在 Model11 测试集平均风险分为 0.9847，但交易统计、图结构、动态资金图谱特征均为 0，高分主要来自账户画像与图基座特征，属于画像层研判，不是资金行为链路证据；账户级五折留出验证 Top5% 召回约 44%，说明这部分识别应作为补数复核线索，不能宣称已生成真实资金链路。
